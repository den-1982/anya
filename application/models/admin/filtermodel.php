...
{
	public $type = array('checkbox', 'radio', 'select', 'group');
	...
	public function getFilter($id = 0)
	{
		...
		if ($filter){
			...
			$categories = $this->db->query('SELECT * 
											...
											WHERE id_filter = '.$filter->id)->result();
			
			$filter->categories = array();
			foreach ($categories as $category){
				$filter->categories[] = $category->id_category;
			}
		}
	...
	public function addFilter()
	{
		...
		$type		= isset($_POST['filter_type']) ? ( in_array($_POST['filter_type'], $this->type, true) ? $_POST['filter_type'] : 'checkbox' ) : 'checkbox';
		$pricing	= isset($_POST['filter_pricing']) ? (int)(boolean)$_POST['filter_pricing'] : 0;
		
		$this->db->query('INSERT INTO filter (name, image, pricing, type) 
							VALUES(
								"'.$name.'",
								"'.$image.'",
								"'.$pricing.'", 
		...
		# значения фильтра
		if (isset($_POST['filter_item_name']['insert'])){
			foreach ($_POST['filter_item_name']['insert'] as $k=>$v){
		...
		if (isset($_POST['id_category'])){
			foreach ($_POST['id_category'] as $k){
				$id_category = abs((int)$k);
				$this->db->query('INSERT INTO category_filter(id_category, id_filter) 
									VALUES(
										"'.$id_category.'", 
										"'.$id_filter.'"
									)');
	...
	public function updateFilter()
	{
		...
		# редактируем старые
		if (isset($_POST['filter_item_name']['update'])){
			foreach ($_POST['filter_item_name']['update'] as $k=>$v){
				$_id	= abs((int)$k);
	...
	public function changeType()
	{
		...
		$type = isset($_POST['type']) ? (in_array($_POST['type'], $this->type, true) ? $_POST['type'] : 'checkbox') : 'checkbox';
		
		$this->db->query('UPDATE filter SET type = "'.$type.'"  WHERE id = '.$id);
...
/* Location: ./application/models/admin/filtermodel.php */